/**
 * Event Intelligence Data Transformation Pipeline
 * 
 * Core Philosophy: Every event has patterns. Find them, use them, profit.
 * Works for: 100 customers (cold start) to 100K customers (enterprise)
 */

class EventIntelligencePipeline {
  /**
   * STAGE 1: Data Ingestion & Normalization
   * Transform messy CSVs into clean, analyzable data
   */
  async ingestCustomerData(source, organizationId) {
    const pipeline = [];
    
    // Stream process for scalability
    await this.streamProcess(source, async (rawRecord) => {
      // Normalize: The same whether 100 or 100K records
      const normalized = {
        // Identity Resolution
        customer_id: this.resolveCustomerId(rawRecord),
        email: this.normalizeEmail(rawRecord.email),
        phone: this.normalizePhone(rawRecord.phone),
        
        // Transaction Intelligence
        purchases: this.extractPurchases(rawRecord),
        total_spent: this.calculateTotalSpent(rawRecord),
        
        // Behavioral Fingerprint
        first_purchase_date: this.parseDate(rawRecord.first_purchase),
        last_purchase_date: this.parseDate(rawRecord.last_purchase),
        event_types_attended: this.extractEventTypes(rawRecord),
        
        // Geographic Intelligence
        location: this.inferLocation(rawRecord),
        travel_pattern: this.calculateTravelPattern(rawRecord)
      };
      
      pipeline.push(this.enrichWithContext(normalized));
    });
    
    return this.batchProcess(pipeline);
  }
  
  /**
   * STAGE 2: Pattern Recognition
   * Find the rhythms in your data
   */
  async extractEventPatterns(customers, events) {
    // Works with ANY data volume
    const patterns = {
      // Temporal Patterns (When do people buy?)
      purchase_curves: await this.analyzePurchaseCurves(customers, events),
      
      // Behavioral Segments (How do they buy?)
      segments: await this.discoverBehavioralSegments(customers),
      
      // Value Patterns (What drives revenue?)
      value_drivers: await this.identifyValueDrivers(customers),
      
      // Event Context (What makes each event unique?)
      event_profiles: await this.profileEvents(events)
    };
    
    // Cold start? Use industry benchmarks + basic demographics
    if (customers.length < 100) {
      patterns.benchmarks = await this.getIndustryBenchmarks(events[0]?.type);
      patterns.inference_confidence = 'low';
    } else {
      patterns.inference_confidence = customers.length > 1000 ? 'high' : 'medium';
    }
   ...
   */
  async generateIntelligence(patterns, campaignGoal, budget) {
    const intelligence = {
      // WHO: Audience Recommendations
      audiences: this.generateAudienceStrategy(patterns, campaignGoal),
      
      // WHEN: Timing Optimization
      timing: this.optimizeCampaignTiming(patterns.purchase_curves, campaignGoal),
      
      // HOW: Channel & Message Strategy
      strategy: this.developChannelStrategy(patterns, budget),
      
      // WHAT IF: Predictions & Scenarios
      predictions: this.generatePredictions(patterns, campaignGoal, budget)
    };
    
    // Adapt complexity to data availability
    if (patterns.inference_confidence === 'low') {
      // Cold start: Simple, benchmark-based recommendations
      intelligence.recommendations = this.getSimpleRecommendations(campaignGoal);
      intelligence.confidence_note = "Based on industry patterns. Will improve with your data.";
    } else {
      // Data-rich: Sophisticated, pattern-based recommendations
      intelligence.recommendations = this.getSophisticatedRecommendations(patterns);
      intelligence.confidence_note = `Based on ${patterns.segments.length} behavioral patterns found.`;
    }
   ...
   * Convert insights into platform-specific targeting
   */
  async translateToPlatforms(intelligence, platforms = ['meta']) {
    const translations = {};
    
    for (const platform of platforms) {
      translations[platform] = await this[`translateTo${this.capitalize(platform)}`](intelligence);
    }
   ...
   * Meta Translation
   */
  async translateToMeta(intelligence) {
    const config = {
      audiences: [],
      exclusions: []
    };
    
    // If we have customer lists (enterprise)
    if (intelligence.audiences.customAudiences?.length > 0) {
      config.audiences.push(...intelligence.audiences.customAudiences.map(aud => ({
        type: 'custom',
        name: aud.name,
        data: aud.hashedData,
        lookalike: {
    ...
    // Always provide interest/behavior targeting (works for everyone)
    config.audiences.push({
      type: 'interests_behaviors',
      targeting: {
        interests: intelligence.audiences.interests || this.getDefaultInterests(intelligence.eventType),
        behaviors: intelligence.audiences.behaviors || this.getDefaultBehaviors(intelligence.goal),
        demographics: intelligence.audiences.demographics || this.getDefaultDemographics()
      }
    });
    
    // Timing recommendations (universal value)
    config.scheduling = {
      start_date: intelligence.timing.optimal_start,
      end_date: intelligence.timing.optimal_end,
      budget_pacing: intelligence.timing.budget_curve,
      dayparting: intelligence.timing.hourly_optimization
    };
  ...
  async analyzePurchaseCurves(customers, events) {
    // Group purchases by days before event
    const curves = {};
    
    events.forEach(event => {
      curves[event.id] = {
        velocity: [], // Purchases per day leading to event
        cumulative: [], // Cumulative % sold
        acceleration_points: [] // Where sales spike
      };
      
      // Calculate purchase timing for this event
      const eventPurchases = customers
        .flatMap(c => c.purchases)
        .filter(p => p.event_id === event.id);
      
      // Build the curve
      for (let daysBefore = 90; daysBefore >= 0; daysBefore--) {
        const dayPurchases = eventPurchases.filter(p => 
          this.daysBetween(p.purchase_date, event.date) === daysBefore
        );
        
        curves[event.id].velocity.push({
      ...
      // Find acceleration points (where to increase spend)
      curves[event.id].acceleration_points = this.findAccelerationPoints(curves[event.id].velocity);
    });
    
    return curves;
  }
  
  async discoverBehavioralSegments(customers) {
    // Core segments that work for any volume
    const segments = [];
    
    // Calculate behavioral metrics for each customer
    const enrichedCustomers = customers.map(customer => ({
      ...customer,
      metrics: {
        advance_purchase_days: this.median(customer.purchases.map(p => p.days_before_event)),
        average_spend: customer.total_spent / customer.purchases.length,
        event_diversity: new Set(customer.purchases.map(p => p.event_type)).size,
        purchase_frequency: customer.purchases.length / this.accountAgeMonths(customer),
        group_size: this.average(customer.purchases.map(p => p.quantity))
      }
    }));
    
    // Define segments based on behavioral rules
    segments.push({
      id: 'early_planners',
      name: 'Early Bird Planners',
      definition: c => c.metrics.advance_purchase_days > 30,
      members: enrichedCustomers.filter(c => c.metrics.advance_purchase_days > 30),
      intelligence: {
        optimal_campaign_start: -60,
        messaging: 'Early bird pricing available',
        expected_conversion_window: [-45, -30]
      }
    });
    ...
    segments.push({
      ...
      definition: c => c.metrics.advance_purchase_days < 7,
      members: enrichedCustomers.filter(c => c.metrics.advance_purchase_days < 7),
      intelligence: {
        optimal_campaign_start: -10,
        messaging: 'Final tickets available',
        expected_conversion_window: [-7, -1]
      }
    });
    ...
    segments.push({
      id: 'high_value',
      name: 'VIP Spenders',
      definition: c => c.metrics.average_spend > this.percentile(enrichedCustomers, 'average_spend', 80),
      members: enrichedCustomers.filter(c => 
        c.metrics.average_spend > this.percentile(enrichedCustomers, 'average_spend', 80)
      ),
      intelligence: {
        optimal_campaign_start: -45,
        messaging: 'Exclusive VIP experience',
        expected_conversion_window: [-40, -20]
      }
    });
    
    // More segments based on data volume...
    
    return segments.filter(s => s.members.length >= 10); // Only return meaningful segments
  }
  ...
  getIndustryBenchmarks(eventType) {
    // When you have no data, use industry patterns
    const benchmarks = {
      music_festival: {
        typical_purchase_curve: [
          { days_before: 60, cumulative_sold: 0.15 },
          { days_before: 30, cumulative_sold: 0.45 },
          { days_before: 7, cumulative_sold: 0.80 },
          { days_before: 0, cumulative_sold: 0.95 }
        ],
        audience_interests: ['Music festivals', 'Live entertainment', 'Concerts'],
        typical_age_range: '18-34',
        optimal_campaign_window: { start: -60, peak: -30, end: -7 }
      },
      business_conference: {
        typical_purchase_curve: [
          { days_before: 90, cumulative_sold: 0.25 },
          { days_before: 60, cumulative_sold: 0.50 },
          { days_before: 30, cumulative_sold: 0.75 },
          { days_before: 7, cumulative_sold: 0.90 }
        ],
        audience_interests: ['Business conferences', 'Professional development', 'Industry events'],
        typical_age_range: '25-54',
        optimal_campaign_window: { start: -90, peak: -45, end: -14 }
      }
      // More event types...
    };
    
    return benchmarks[eventType] || benchmarks.business_conference;
  }
  ...
  findAccelerationPoints(velocityCurve) {
    // Find where purchase velocity increases significantly
    const points = [];
    
    for (let i = 1; i < velocityCurve.length; i++) {
      const currentVelocity = velocityCurve[i].purchases;
      const previousVelocity = velocityCurve[i-1].purchases;
      
      if (currentVelocity > previousVelocity * 1.5) {
        points.push({
          days_before: velocityCurve[i].days_before,
          velocity_increase: currentVelocity / previousVelocity,
