 ...
 * The foundation - works with minimal or rich data
 */
const CustomerIntelligenceSchema = new Schema({
  // Core Identity
  _organization: { type: Schema.Types.ObjectId, ref: 'Organization', required: true, index: true },
  _customer: { type: Schema.Types.ObjectId, ref: 'Customer' },
  
  // Identity markers (for matching)
  identity: {
    email: { type: String, lowercase: true, trim: true, index: true },
    email_domain: { type: String, lowercase: true }, // For B2B insights
    phone: { type: String, index: true },
    external_id: String, // Their CRM ID
    merge_keys: [String] // For deduplication
  },
  
  // Behavioral DNA (builds over time)
  behavior_profile: {
    // Purchase Mechanics
    ...
    // Timing Patterns
    typical_advance_purchase_days: Number, // Median
    purchase_day_of_week_preference: [Number], // [0-6], Sunday = 0
    purchase_hour_preference: [Number], // [0-23]
    
    // Event Preferences
    event_types: [String], // ['music', 'conference', 'sports']
    price_tier_preference: String, // 'budget', 'standard', 'premium'
    group_behavior: {
      typically_alone: Boolean,
      average_group_size: Number,
      is_coordinator: Boolean // Buys for others
    },
    
    // Response Patterns
    marketing_responsiveness: {
      clicks_without_purchase: { type: Number, default: 0 },
      email_engagement_rate: Number,
      optimal_send_time: Number, // Hour of day
      channel_preference: String // 'email', 'sms', 'social'
    }
  },
  
  // Segments (can belong to multiple)
  segments: [{
    segment_id: String,
    segment_name: String,
    confidence: Number, // 0-1, how well they fit
    assigned_date: Date,
    is_primary: Boolean
  }],
  ...
  intelligence: {
    // Value Scores
    ltv_percentile: Number, // 0-100, where they rank
    growth_trajectory: String, // 'rising', 'stable', 'declining'
    
    // Engagement Scores
    engagement_level: String, // 'highly_engaged', 'engaged', 'passive', 'churning'
    churn_risk_score: Number, // 0-1
    
    // Campaign Scores
    ad_responsiveness: Number, // 0-1, likelihood to respond to ads
    roi_score: Number, // Expected ROI from advertising to them
    
    // Network Effects
    influence_score: Number, // Based on group coordination behavior
    referral_potential: Number // 0-1
  },
  ...
  platform_data: {
    meta: {
      can_match: Boolean, // Has email/phone
      hashed_email: String,
      hashed_phone: String,
      audience_ids: [String], // Which Meta audiences they're in
      excluded: Boolean,
      exclusion_reason: String
    },
    google: {
      can_match: Boolean,
      customer_match_ready: Boolean,
      list_ids: [String]
    },
    enrichment: {
  ...
  // Journey Tracking (builds progressively)
  journey: {
    stage: { 
      type: String, 
      enum: ['unknown', 'aware', 'interested', 'engaged', 'customer', 'advocate'],
      default: 'unknown'
    },
    touchpoints: [{
      timestamp: Date,
...
CustomerIntelligenceSchema.index({ '_organization': 1, 'behavior_profile.lifetime_value': -1 });
CustomerIntelligenceSchema.index({ '_organization': 1, 'segments.segment_id': 1 });
CustomerIntelligenceSchema.index({ '_organization': 1, 'intelligence.ltv_percentile': -1 });

/**
 * Event Pattern Model
 * Captures the "heartbeat" of each event
 */
const EventPatternSchema = new Schema({
  _organization: { type: Schema.Types.ObjectId, ref: 'Organization', required: true, index: true },
  _event: { type: Schema.Types.ObjectId, ref: 'Event', required: true, index: true },
  
  // Event Context
  event_details: {
    name: String,
    date: Date,
    type: String, // 'conference', 'festival', 'workshop'
    category: String, // 'tech', 'music', 'business'
    capacity: Number,
    price_range: {
      min: Number,
      max: Number,
  ...
  sales_pattern: {
    ...
    milestones: {
      on_sale_date: Date,
      sold_25_percent_date: Date,
      sold_50_percent_date: Date,
      sold_75_percent_date: Date,
      sold_out_date: Date
    },
    ...
    surge_periods: [{
      ...
      trigger: String // 'early_bird_ending', 'artist_announcement', etc.
    }]
  },
  ...
  marketing_performance: {
    ...
    channel_roi: [{
      channel: String, // 'meta', 'google', 'email'
      spend: Number,
    ...
    // Campaign windows
    effective_campaign_windows: [{
      channel: String,
      start_days_before: Number,
      end_days_before: Number,
      peak_days_before: Number,
      budget_allocation_percentage: Number
    }],
    
    // Creative insights
    top_performing_messages: [{
      message_theme: String, // 'early_bird', 'last_chance', 'exclusive'
      conversion_rate: Number,
      optimal_timing_days_before: Number
    }]
  },
  
  // Predictions (for similar future events)
  pattern_predictions: {
    confidence_level: String, // 'high', 'medium', 'low'
    predicted_sales_curve: [{
      days_before: Number,
      expected_percentage_sold: Number,
      confidence_interval: { low: Number, high: Number }
    }],
    recommended_marketing_calendar: [{
      days_before: Number,
      action: String,
      channel: String,
  ...
  // Learning metadata
  analysis_metadata: {
    data_points: Number, // Number of transactions analyzed
    pattern_stability: Number, // 0-1, how consistent the pattern is
    last_updated: Date,
...
EventPatternSchema.index({ '_organization': 1, '_event': 1 }, { unique: true });
EventPatternSchema.index({ '_organization': 1, 'event_details.type': 1 });

/**
 * Campaign Intelligence Model
 * Tracks what actually works
 */
const CampaignIntelligenceSchema = new Schema({
  _organization: { type: Schema.Types.ObjectId, ref: 'Organization', required: true, index: true },
  _campaign: { type: Schema.Types.ObjectId, ref: 'Campaign', required: true },
  ...
  audience_details: {
    ...
    custom_audience_used: Boolean,
    lookalike_used: Boolean,
    targeting_criteria: Schema.Types.Mixed // Platform-specific
  },
  ...
  insights: {
    ...
    best_creative_theme: String,
    
    // What didn't
    waste_factors: [String],
    excluded_segments_that_clicked: Number,
  ...
  attribution: {
    model_used: String, // 'last_touch', 'linear', 'data_driven'
    touch_points: [{
      channel: String,
      timestamp: Date,
 ...
 */
const SegmentDefinitionSchema = new Schema({
  _organization: { type: Schema.Types.ObjectId, ref: 'Organization', required: true, index: true },
  ...
  definition: {
    // Rule-based criteria
    rules: [{
      field: String, // 'behavior_profile.lifetime_value'
      operator: String, // 'gt', 'lt', 'eq', 'in', 'between'
      value: Schema.Types.Mixed
    }],
    
    // ML-based criteria (future)
    ml_cluster_id: String,
  ...
  intelligence: {
    ...
    value_concentration: Number, // % of revenue
    growth_rate: Number, // Month-over-month
    
    // Behavioral summary
    typical_advance_purchase_days: Number,
    average_ltv: Number,
...
// Compile models
const CustomerIntelligence = mongoose.model('CustomerIntelligence', CustomerIntelligenceSchema);
const EventPattern = mongoose.model('EventPattern', EventPatternSchema);
const CampaignIntelligence = mongoose.model('CampaignIntelligence', CampaignIntelligenceSchema);
